...
BOOL CProductNewDlg::OnInitDialog()
{
	...
	////////////////////////////////////////页面4
	CProductStep3Dlg *pStep4=new CProductStep3Dlg();
	if(pStep4==NULL)
	{
	...
	pStep4->Create(IDD_PRODUCTSTEP3_DLG,this);
	...
	////////////////////////////////////////页面5
	CProductStep2Dlg *pStep5=new CProductStep2Dlg();
	if(pStep5==NULL)
	{
	...
	pStep5->Create(IDD_PRODUCTSTEP2_DLG,this);
	...
	////////////////////////////////////////页面6
	CProductStep5Dlg *pStep6=new CProductStep5Dlg();
	if(pStep6==NULL)
	{
	...
	pStep6->Create(IDD_PRODUCTSTEP5_DLG,this);
...
{
	// TODO: Add your control notification handler code here
	if (((CProductStep5Dlg*)m_pPageList[6])->OnWizardNext()==-1)
		return;
	else
	{
		CSetWeighDlg dlg;
...
void CProductNewDlg::ShowPage(UINT nPos) 
{
	...
	else if(nPos>(UINT)m_nCurrentPage)
	{
		...
		case 3:
			...
			((CProductStep3Dlg*)m_pPageList[nPos])->m_ProductInfo=((CProductStep0Dlg*)m_pPageList[0])->m_ProductInfo;
...
void CProductNewDlg::GetIndexVal(VectorXd& dA1,VectorXd& dA2)
{
	...
	n4=((CProductStep3Dlg*)m_pPageList[4])->m_ListCtrlItem.size();
	n5=((CProductStep2Dlg*)m_pPageList[5])->m_ListCtrlItem.size();
	n6=((CProductStep5Dlg*)m_pPageList[6])->m_ListCtrlItem.size();
	vector<double> TechMaturyVal,TechIndexVal,ThreeProVal,ImpactVibVal,MatInfoVal,EconomyVal;
	...
	for(int i=0;i<n4;++i)
	{
		ImpactVibVal.push_back(_ttol(((CProductStep3Dlg*)m_pPageList[4])->m_ListCtrlItem[i].m_IndexScore));     //cstring转double

	}
	for(int i=0;i<n5;++i)
	{
		MatInfoVal.push_back(_ttol(((CProductStep2Dlg*)m_pPageList[5])->m_ListCtrlItem[i].m_MatScore));     //cstring转double

	}
	for(int i=0;i<n6;++i)
	{
		EconomyVal.push_back(_ttol(((CProductStep5Dlg*)m_pPageList[6])->m_ListCtrlItem[i].m_IndexScore));     //cstring转double
...
void CProductNewDlg::SaveResultInfo(vector<CLowValItem>& m_LowValItem,vector<CIndexValItem>& m_IndexVal)
{
	...
	for (int i=0;i<((CProductStep3Dlg*)m_pPageList[4])->m_LowValItem.size();++i)
	{
		CString str;
		str.Format(CString("%d"),nLowValItemNum);
		CLowValItem OneItem;
		OneItem=((CProductStep3Dlg*)m_pPageList[4])->m_LowValItem[i];
		OneItem.m_Item=str;  //序号重新赋值
		m_LowValItem.push_back(OneItem);
		++nLowValItemNum;
	}
	for (int i=0;i<((CProductStep2Dlg*)m_pPageList[5])->m_LowValItem.size();++i)
	{
		CString str;
		str.Format(CString("%d"),nLowValItemNum);
		CLowValItem OneItem;
		OneItem=((CProductStep2Dlg*)m_pPageList[5])->m_LowValItem[i];
		OneItem.m_Item=str;  //序号重新赋值
		m_LowValItem.push_back(OneItem);
		++nLowValItemNum;
	}
	for (int i=0;i<((CProductStep5Dlg*)m_pPageList[6])->m_LowValItem.size();++i)
	{
		CString str;
		str.Format(CString("%d"),nLowValItemNum);
		CLowValItem OneItem;
		OneItem=((CProductStep5Dlg*)m_pPageList[6])->m_LowValItem[i];
		OneItem.m_Item=str;  //序号重新赋值
		m_LowValItem.push_back(OneItem);
		++nLowValItemNum;
	}
