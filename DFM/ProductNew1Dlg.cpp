// ProductNew1Dlg.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CProductNew1Dlg, CDialogEx)

CProductNew1Dlg::CProductNew1Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CProductNew1Dlg::IDD, pParent)
...
CProductNew1Dlg::~CProductNew1Dlg()
{
...
void CProductNew1Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_BUTTON_CHART10, m_btnChart10);
	DDX_Control(pDX, IDC_BUTTON_CHART11, m_btnChart11);
	DDX_Control(pDX, IDC_BUTTON_CHART12, m_btnChart12);
	DDX_Control(pDX, IDC_BUTTON_CHART13, m_btnChart13);
	DDX_Control(pDX, IDC_BUTTON_CHART14, m_btnChart14);
	DDX_Control(pDX, IDC_BUTTON_CHART15, m_btnChart15);
}


BEGIN_MESSAGE_MAP(CProductNew1Dlg, CDialogEx)
	ON_WM_CTLCOLOR()
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_EVALPRE1, &CProductNew1Dlg::OnBnClickedEvalpre1)
	ON_BN_CLICKED(IDC_EVALNEXT1, &CProductNew1Dlg::OnBnClickedEvalnext1)
	ON_BN_CLICKED(IDC_EVALIN1, &CProductNew1Dlg::OnBnClickedEvalin1)
	ON_WM_SIZE()
END_MESSAGE_MAP()

//控件大小位置变化
BEGIN_EASYSIZE_MAP(CProductNew1Dlg, CDialogEx) 
	EASYSIZE(IDC_STATIC_PLANE1,ES_BORDER,ES_BORDER,ES_BORDER,ES_BORDER,0)
END_EASYSIZE_MAP 
// CProductNew1Dlg message handlers


BOOL CProductNew1Dlg::OnInitDialog()
{
	CDialogEx::OnInitDialog();

	// TODO:  Add extra initialization here
	//获取子对话框的包容矩形
	GetDlgItem(IDC_STATIC_PLANE1)->GetWindowRect(&m_rectPanel);
	...
	////////////////////////////////////////页面4
	CProductStep7Dlg *pStep4=new CProductStep7Dlg();
	if(pStep4==NULL)
	{
	...
	pStep4->Create(IDD_PRODUCTSTEP7_DLG,this);
	...
	short	shBtnColor = 30;
	m_btnChart10.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btnChart11.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btnChart12.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btnChart13.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btnChart14.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
	m_btnChart15.OffsetColor(CButtonST::BTNST_COLOR_BK_IN, shBtnColor);
...
void CProductNew1Dlg::OnSize(UINT nType, int cx, int cy)
{
	...
	LockWindowUpdate();
	CWnd *pWnd;         //调整子窗口大小
	pWnd=GetDlgItem(IDC_STATIC_PLANE1);
...
HBRUSH CProductNew1Dlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
...
void CProductNew1Dlg::OnDestroy()
{
	CDialogEx::OnDestroy();
...
void CProductNew1Dlg::OnBnClickedEvalpre1()
{
...
void CProductNew1Dlg::OnBnClickedEvalnext1()
{
	// TODO: Add your control notification handler code here
...
void CProductNew1Dlg::OnBnClickedEvalin1()
{
	 //TODO: Add your control notification handler code here
	if (((CProductStep5Dlg*)m_pPageList[5])->OnWizardNext()==-1)
		return;
	else
	{
		CSetWeighDlg dlg;
...
//根据页面情况设置按钮状态
void CProductNew1Dlg::SetWizButton(UINT nPos)
{
	GetDlgItem(IDC_EVALPRE1)->EnableWindow(TRUE);
	GetDlgItem(IDC_EVALNEXT1)->EnableWindow(TRUE);
	GetDlgItem(IDC_EVALIN1)->EnableWindow(TRUE);

	switch(nPos)
	{
	case 0: //第一步(上一步失效)
		GetDlgItem(IDC_EVALPRE1)->EnableWindow(FALSE);
		GetDlgItem(IDC_EVALIN1)->EnableWindow(FALSE);
		break;
	case 1: //中间步
		GetDlgItem(IDC_EVALIN1)->EnableWindow(FALSE);
		break;
	...
	case 4://中间步
		...
		break;
	case 5://最后一步
		GetDlgItem(IDC_EVALNEXT1)->EnableWindow(FALSE);
		break;
	}
}
...
//显示nPos页面
void CProductNew1Dlg::ShowPage(UINT nPos) 
{
	//显示第一页,隐藏当前页，激活下一页
	...
	else if(nPos>(UINT)m_nCurrentPage)
	{
		...
		case 3:
			if(((CProductStep4Dlg*)m_pPageList[m_nCurrentPage])->OnWizardNext()==-1)
				return;
			((CProductStep7Dlg*)m_pPageList[nPos])->m_MatInfoList.DeleteAllItems();
			((CProductStep7Dlg*)m_pPageList[nPos])->m_ProductInfo=((CProductStep0Dlg*)m_pPageList[0])->m_ProductInfo;
			((CProductStep7Dlg*)m_pPageList[nPos])->ReadTechChart();
			((CProductStep7Dlg*)m_pPageList[nPos])->m_ListCtrlItem.clear();
			((CProductStep7Dlg*)m_pPageList[nPos])->InitChartInfo();

			((CProductStep7Dlg*)m_pPageList[nPos])->OnWizardActive();
			break;
		case 4:
			if(((CProductStep7Dlg*)m_pPageList[m_nCurrentPage])->OnWizardNext()==-1)
				return;
	...
	else
	{
		...
		case 3:
			...
			break;
		case 4:
			if(((CProductStep7Dlg*)m_pPageList[m_nCurrentPage])->OnWizardPrevious()==-1)
				return;
			((CProductStep4Dlg*)m_pPageList[nPos])->OnWizardActive();
			break;
		case 5:
...
//设置文本状态显示变化
void CProductNew1Dlg::SetTextShow(UINT nPos)
{

	switch(nPos)
	{
	case 0:
		m_btnChart10.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(255, 0, 0));
		m_btnChart11.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart12.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart13.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart14.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart15.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		break;
	case 1:
		m_btnChart10.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart11.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(255, 0, 0));
		m_btnChart12.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart13.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart14.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart15.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
	...
	case 2:
		m_btnChart10.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart11.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart12.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(255, 0, 0));
		m_btnChart13.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart14.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart15.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		break;
	case 3:
		...
		m_btnChart11.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart12.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart13.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(255, 0, 0));
		m_btnChart14.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart15.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		break;
	case 4:
		...
		m_btnChart12.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart13.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart14.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(255, 0, 0));
		m_btnChart15.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		break;
	case 5:
		m_btnChart10.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		...
		m_btnChart13.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart14.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart15.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(255, 0, 0));
		break;
	default:
		m_btnChart10.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
		m_btnChart11.SetColor(CButtonST::BTNST_COLOR_FG_OUT, RGB(0, 0, 0));
...
///////////////////////////////////////////////////////////////////模糊层次分析法计算相关
void CProductNew1Dlg::GetIndexVal(VectorXd& dA1,VectorXd& dA2)
{
	...
	n4=((CProductStep7Dlg*)m_pPageList[4])->m_ListCtrlItem.size();
	n5=((CProductStep5Dlg*)m_pPageList[5])->m_ListCtrlItem.size();
	vector<double> TechMaturyVal,TechIndexVal,ThreeProVal,MatInfoVal,EconomyVal;
	...
	for(int i=0;i<n4;++i)
	{
		MatInfoVal.push_back(_ttol(((CProductStep7Dlg*)m_pPageList[4])->m_ListCtrlItem[i].m_MatScore));     //cstring转double
	}
	for(int i=0;i<n5;++i)
	{
	...
	c4=3+MinVal(MatInfoVal);
	c5=3+MinVal(EconomyVal);
	...
	//保存二级指标评分
	VectorXd vTemp1(5);
	vTemp1<<c1,c2,c3,c4,c5;
	m_dC2=vTemp1;

	VectorXd dC2(4);      //用于构造模糊矩阵
	dC2<<c1,c2,c3,c4;
	...
	double cc2=c5;      //因为经济指标只有一个子指标，故评分直接为该指标评分
...
double CProductNew1Dlg::MinVal(vector<double>& IndexVal)
{
	double dMin=3.0;
...
/////////////////////////结果输出相关
vector<CString>& CProductNew1Dlg::SetResultVal(vector<CString>& m_ItemVal)
{
	//评价最终可制造性
	m_ItemVal.clear();
...
void CProductNew1Dlg::SaveResultInfo(vector<CLowValItem>& m_LowValItem,vector<CIndexValItem>& m_IndexVal)
{
	//综合保存低分项
	m_LowValItem.clear();
	m_IndexVal.clear();
	...
	for (int i=0;i<((CProductStep7Dlg*)m_pPageList[4])->m_LowValItem.size();++i)
	{
		CString str;
		str.Format(CString("%d"),nLowValItemNum);
		CLowValItem OneItem;
		OneItem=((CProductStep7Dlg*)m_pPageList[4])->m_LowValItem[i];
		OneItem.m_Item=str;  //序号重新赋值
		m_LowValItem.push_back(OneItem);
		++nLowValItemNum;
	}
