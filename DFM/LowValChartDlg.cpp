...
BOOL CLowValChartDlg::OnInitDialog()
{
	...
	m_LowValList.InsertColumn(1,CString("评价指标"),LVCFMT_CENTER,width1/6);
	m_LowValList.InsertColumn(2,CString("评语"),LVCFMT_CENTER,width1/12);
	m_LowValList.InsertColumn(3,CString("可改进项"),LVCFMT_CENTER,width1/6);
	m_LowValList.InsertColumn(4,CString("类别"),LVCFMT_CENTER,width1/6);
	m_LowValList.InsertColumn(5,CString("改进建议"),LVCFMT_CENTER,width1/3);
...
void CLowValChartDlg::SetListItem(vector<CLowValItem>& m_LowValItem)
{
	...
	for (int i=0;i<m_LowValItem.size();++i)
	{
		...
		m_LowValList.SetItemText(i,2,m_LowValItem[i].m_IndexComment);
		m_LowValList.SetItemText(i,3,m_LowValItem[i].m_TechEvalIndex);
		m_LowValList.SetItemText(i,4,m_LowValItem[i].m_Classify);
