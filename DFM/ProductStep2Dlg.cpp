...
void CProductStep2Dlg::SetListItem(vector<vector<CString>>& str_AllItem)
{
	...
	MatchMatVal(m_ListCtrlItem,m_ProductInfo);  //匹配分值
}

void CProductStep2Dlg::MatchMatVal(vector<CMatChartItem>& m_ListCtrlItem,CProductInfo& m_ProductInfo)
{
	_RecordsetPtr m_pRs;
	vector<vector<CString>> m_MatMatchInfo;       //数据库中的匹配信息
	CString sql = CString("select * from Material where EvalModelID=")+m_ProductInfo.str_EvalType;
...
void CProductStep2Dlg::GetProInfo(CProductInfo& m_ProductInfo)
{
